...
- **Unterstützte Plattform.** Cleanup
...
- **Mehrere Tabs zeigten keine StreamSeeker-Footer/Badges mehr.**
  ...
  _eine_ zentrale SSE-Verbindung und broadcastet Events per
...
- **Gap-Fill in der Detail-Anzeige:** Felder werden jetzt aus _allen_
...
- Click a card in the Sammlung- or Favoriten-Tab to open the series page
  in a new tab (aniworld.to / s.to - URL derived from
  stream + slug).
...
- Content scripts for `aniworld.to` and `s.to` that render
